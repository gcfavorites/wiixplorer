 ...
 */
GuiKeyboardAlone::GuiKeyboardAlone(void)
{
	width = 14*42+40;
	height = 5*42+80;
	shift = false;
	...
	keyBack->Clicked.connect(this, &GuiKeyboardAlone::OnSpecialKeyPress);
	this->Append(keyBack);
	...
	keyTab->Clicked.connect(this, &GuiKeyboardAlone::OnSpecialKeyPress);
	this->Append(keyTab);
	...
	keyCaps->Clicked.connect(this, &GuiKeyboardAlone::OnSpecialKeyPress);
	this->Append(keyCaps);
	...
	keyShift->Clicked.connect(this, &GuiKeyboardAlone::OnSpecialKeyPress);
	...
	keyLang->Clicked.connect(this, &GuiKeyboardAlone::OnSpecialKeyPress);
	this->Append(keyLang);
	...
	keyEnter->Clicked.connect(this, &GuiKeyboardAlone::OnSpecialKeyPress);
	...
	keySpace->Clicked.connect(this, &GuiKeyboardAlone::OnSpecialKeyPress);
 ...
 */
GuiKeyboardAlone::~GuiKeyboardAlone()
{
	delete keyCapsText;
	delete keyCapsImg;
...
void GuiKeyboardAlone::SwitchKeyLanguage()
{
...
void GuiKeyboardAlone::OnSpecialKeyPress(GuiButton *sender, int pointer UNUSED, const POINT &p UNUSED)
{
	if(sender == keySpace)
	{
		keyPressed(L' ');
...
void GuiKeyboardAlone::OnNormalKeyPress(GuiButton *sender, int pointer UNUSED, const POINT &p UNUSED)
{
	for(int i = 0; i < MAXROWS; i++)
	{
		for(int j = 0; j < MAXKEYS; j++)
...
void GuiKeyboardAlone::Update(GuiTrigger * t)
{
	GuiFrame::Update(t);
