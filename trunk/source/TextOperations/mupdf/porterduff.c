...
static void path_1c1(byte * src, byte cov, int len, byte * dst)
{
	while (len--)
	{
		cov += *src; *src = 0; src++;
...
static void path_1o1(byte * src, byte cov, int len, byte * dst)
{
	while (len--)
	{
		cov += *src; *src = 0; src++;
...
// With 4 In 1 Over 4
static void path_w4i1o4(byte * argb, byte * src, byte cov, int len, byte * dst)
{
	byte alpha = argb[0];
	byte r = argb[1];
	byte g = argb[2];
...
static void text_w4i1o4(byte * argb, byte * src0, int srcw, byte * dst0, int dstw, int w0, int h)
{
