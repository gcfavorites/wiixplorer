...
/**
 * Constructor for the GuiTextConsole class.
 */
GuiTextConsole::GuiTextConsole(const char * t, int s, GXColor c)
{
	origText = NULL;
	text = NULL;
	size = s;
...
/**
 * Constructor for the GuiTextConsole class, uses presets
 */
GuiTextConsole::GuiTextConsole(const char * t)
{
	origText = NULL;
	text = NULL;
	size = presetSize;
...
/**
 * Destructor for the GuiTextConsole class.
 */
...
void GuiTextConsole::SetText(const char * t)
{
    if(t && origText)
...
void GuiTextConsole::SetTextf(const char *format, ...)
{
	char *tmp=0;
	va_list va;
	va_start(va, format);
...
void GuiTextConsole::SetPresets(int sz, GXColor c, int w, u16 s, int h, int v)
{
  	presetSize = sz;
	presetColor = c;
	presetStyle = s;
...
void GuiTextConsole::SetFontSize(int s)
{
 
	size = s;
}

void GuiTextConsole::SetMaxWidth(int width, int w)
{
 	maxWidth = width;
	wrapMode = w;
...
void GuiTextConsole::SetColor(GXColor c)
{
	color = c;
	alpha = c.a;
}

void GuiTextConsole::SetStyle(u16 s)
{
 	style = s;
}

void GuiTextConsole::SetAlignment(int hor, int vert)
{
	style = 0;
...
int GuiTextConsole::GetTextWidth()
{
	if(!text)
		return 0;
...
int GuiTextConsole::GetTextWidth(int ind)
{
	if(!textDynRow[ind])
 ...
 */
void GuiTextConsole::SetFirstLine(int line)
{
	firstLine = line;
...
void GuiTextConsole::SetLinesToDraw(int line)
{
 	linestodraw = line;
...
int GuiTextConsole::GetTotalLines()
{
    return totalLines-1;
}

int GuiTextConsole::GetTextMaxWidth()
{
    return maxWidth;
}

wchar_t * GuiTextConsole::GetDynText()
{
    if(!text)
        return textDyn;
...
wchar_t * GuiTextConsole::GetDynTextLine(int ind)
{
    if(!textDynRow[ind])
...
u32 GuiTextConsole::GetLineBreakOffset(int ind)
{
    if(!LineBreak)
        return 0;
...
const char * GuiTextConsole::GetOrigText()
{
    return origText;
}
 ...
 * Change font
 */
bool GuiTextConsole::SetFont(const u8 *fontbuffer, const u32 filesize)
{
...
/*
	...
	if(maxWidth > 0 && maxWidth <= textWidth)
	{
		...
		else
		{
			...
			[currentSize])->drawText(this->GetLeft(), this->GetTop(), text, c, style);
		}
	}
	else
	{
