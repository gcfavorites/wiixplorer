...
#else
		have_samples = 0;
		ASND_SetVoice(0,VOICE_STEREO_16BIT,48000,0,(void*)OutputBuffer[CurrentBuffer],ADMA_BUFFERSIZE,mp3_volume,mp3_volume,DataTransferCallback);
#endif
...
static void DataTransferCallback()
{
	...
	if(have_samples==1) {
		if(ASND_AddVoice(0,(void*)OutputBuffer[CurrentBuffer],ADMA_BUFFERSIZE)==SND_OK) {
			have_samples = 0;
			CurrentBuffer ^= 1;
		}
	}
	if(!(ASND_TestPointer(0,(void*)OutputBuffer[CurrentBuffer]) && ASND_StatusVoice(0)!= SND_UNUSED)) {
		if(have_samples==0) {
			MP3Playing = (buf_get(&OutputRingBuffer,OutputBuffer[CurrentBuffer],ADMA_BUFFERSIZE)>0);
			have_samples = 1;
		}
